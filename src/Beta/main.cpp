...
int main(int argc, char* argv[]) {
    ...
    const std::string VERSION = "1.0.0"; // 工具版本号
    static std::vector<std::string> libraries_to_install;
    // 解析命令行参数
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-I" || arg == "--install") {
            if (i + 1 < argc) {
                ...
                std::cerr << "Error: Missing library name after -I" << std::endl;
                return 1;
            }
        }
            ...
            std::cout << "Usage: " << argv[0] << " [options]\n"
                      ...
                      << "  -v  --version Version Output the version of the program\n"
